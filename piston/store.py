from . import oauth
from .models import Consumer, generate_random, Nonce, Token, VERIFIER_SIZE
...
class DataStore(oauth.OAuthDataStore):
    ...
    def fetch_request_token(self, oauth_consumer, oauth_callback):
        if oauth_consumer.key == self.consumer.key:
            self.request_token = Token.objects.create_token(
                consumer=self.consumer, token_type=Token.REQUEST, timestamp=self.timestamp
            )
    ...
    def fetch_access_token(self, oauth_consumer, oauth_token, oauth_verifier):
        if (
            ...
            and (not oauth_verifier or oauth_verifier == self.request_token.verifier)
